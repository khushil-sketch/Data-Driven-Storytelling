...
```{r}
## GETTING THE SEASONAL THEFT COUNTS AS A PERCENTAGE OF YEARLY THEFT COUNTS
...
# We have to Join the yearly counts with the seasonal counts to then be able to mathematical operations on the 2 different column values
thefts_percentage_by_season <- thefts_percentage_by_season %>%
  inner_join(yearly_thefts_by_premises, by = "PREMISES_TYPE") %>%
  mutate("% of Yearly Count" = round((seasonal_count / yearly_count) * 100))



thefts_percentage_by_season
...
ggplot(thefts_by_day_of_week, aes(x = OCC_DOW, y = count, fill = OCC_DOW)) +
  ...
  labs(title = "Bikes Stolen each Day | Highest to Lowest",
       ...
       y = "") +
  theme_minimal() +
  theme(axis.title.x = element_text(face = "bold", family = "Sans Serif", size = 20, color="white"),
        axis.title.y = element_text(face = "bold", family = "mono", size = 14),
        plot.title = element_text(face = "bold", family = "sans serif", size = 18, hjust = 0.5, color="white"),
        plot.background = element_rect(fill = "black", colour = "black"), # Set plot background to black
        axis.text.x = element_text(face = "bold", family = "mono", size = 16, color="#e0e0e0"),
        axis.text.y = element_text(face = "bold", family = "mono", size = 16, color="#e0e0e0"),
        legend.position = "none", # Remove legend
        panel.grid.major = element_blank(),  # Removes major grid lines
        panel.grid.minor = element_blank()) + # Removes minor grid lines
  coord_flip() + # Makes the plot horizontal
  geom_image(data = thefts_by_day_of_week, aes(x = OCC_DOW, y = count, image = "C:\\Users\\USER\\OneDrive - University of Toronto\\UofT Classes\\STA313 A2\\bike-128.ico"),size = 0.09, hjust = 0) # Adjust size and position as needed
